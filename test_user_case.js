function* jps(grid, start, end, rows, cols) {
  ...
  while (openSet.length > 0) {
    ...
    if (current.x === end.x && current.y === end.y) {
      ...
      path.reverse();
      for (const step of path) yield { ...step, type: 'path' };
      return;
    }
...
while (true) {
  ...
  if (result.value && result.value.type === 'path') Object.assign(result.value, {type: 'path'}), path.push(result.value);
  else if (result.value) visited.push(result.value);
}
